...
SQLPersonal = "SELECT row_number() OVER(Order By r.rollno ) AS [Sl], p.id as [ID] ,(p.FirstName+ ' '+p.LastName) as [Name] , p.contactphone,p.email , r.rollno, r.personalid,r.trycount FROM roll r inner join jobpos j on j.personalid = r.personalid inner join personal p on p.id = j.personalid  Where r.postedon>'04/22/2018' and r.trycount = 0  and r.personalid <> 0 and r.jobid = 1 and j.jobid = 1 "
...
strMessage1 = "<table class='table'><thead><tr><th>Not able download list of applicants Assistant Cash Officer </th></tr></thead></table><table class='table'><thead><th>SL</th><th>Login ID</th><th>Name</th><th>Contact Phone</th><th>Email</th><th>Roll no</th></thead><tbody>"
