...
<div style="padding-top:100px; padding-left:50px; padding-bottom:50px; border:2px solid #93F; border-radius:5px;" align="center">
 ...
 <form action="?<%=Encode("Location=Admin&FormAction=Roll_Delete_All")%>" method="post" name="frmResume" id="frmResume">
...
strIn = "  p.id  in (select personalid from roll where  jobid = "&strPosition&"  ) "
	...
	If arrUserInfo(0,0) <> "Nothing" Then 
			...
			strSqlinsert = "" 
			strSqlinsert = "update jobpos set shortlisted = 0  where jobid = "&strPosition
			Response.Write "</br>"& strSqlinsert 'strSqlinsert
			objERS.ExecuteQuery strSqlinsert, errMsg
						
			strSqlinsert = "" 
			strSqlinsert = "delete from Roll where jobid = "&strPosition
			Response.Write "</br>"& strSqlinsert 'strSqlinsert
			objERS.ExecuteQuery strSqlinsert, errMsg
