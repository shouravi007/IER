...
SQLPersonal = "SELECT convert(date , postedon) as AppDate, COUNT(PERSONALID), COUNT(CASE WHEN JOBID = 6 THEN PERSONALID END) As [JobID-6], COUNT(CASE WHEN JOBID = 7 THEN PERSONALID END) As [JobID-7] FROM roll Where postedon>'04/22/2018' and trycount <> 0 and jobid = 1 GROUP BY convert(date , postedon)"
...
' Response.Write("<br/>Array: " & Ubound(arrPersonal,2)) 
	...
	Next
	strMessage1 = strMessage1 & "<div class='row job-list-row hidden-xs'><div class='col-sm-5'><h3 class='title'>Grand Total</h3></div><div class='col-sm-4'><h3 class='title'>" & varSO & "</h3></div><div class='col-sm-3'><h3 class='title'><span title='Trainee Officer'>" & varTotal & "</span></h3></div></div></div></div></div>" 
End If %>
