...
<%
	strModuleName = "View ShortlistMTO" 
	strDescription = "view shortlist candidates list for mto : "&strUserName&"##"&strPassword
...
<!--#include File = "UserLog.asp" -->
  ...
  <%
   ...
   if rest = "" then
   sql = "select top 1500 p.id,j.trackingno,p.photourl,(p.firstname+ ' '+p.lastname) as name,p.cellphone,p.email,r.rollno from personal p inner join jobpos j on j.personalid = p.id inner join roll r on r.personalid = p.id  where p.shortlisted = 1 and p.jobid = 6 and j.change is null and r.jobid = 6 order by r.rollno asc  "
   else
   sql = "SELECT * FROM (  select  p.id,j.trackingno,p.photourl,(p.firstname+ ' '+p.lastname) as name,p.cellphone,p.email ,r.rollno , ROW_NUMBER() OVER (order by r.rollno asc )  as rownum from personal p inner join jobpos j on j.personalid = p.id  inner join roll r on r.personalid = p.id where p.shortlisted = 1 and p.jobid = 6 and j.change is null and r.jobid = 6 ) seq WHERE seq.rownum > 1500 "
   end if 
  ...
  %>
				...
				<div class="col-sm-12">
					<div class="login-form bg-style">
							...
							<fieldset>
								<legend>Applicants List  for MTO(Shortlisted):</legend>
								<div class="row">
									<div class="col-sm-12">
