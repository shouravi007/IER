...
<tr>
   <td colspan="3">Position
        <select id="txtPosition" name="txtPosition" >	
         ...
         <option value="2">Trainee Officer</option> 
          <option value="6">Trainee Cash Officer</option>
         </select></td>
   <td>&nbsp;</td>
   <td>&nbsp;</td>
   <td>&nbsp;</td>
...
'Dim strQuery
	...
	If InStr(Request.ServerVariables("SERVER_NAME"), "serverbdj") > 0  Or  InStr(Request.ServerVariables("SERVER_NAME"), "server-ers") > 0  Then  
		...
		strPath = server.mappath("/primebank/default/body/attendance") & "/" & "Room.xls"
	Else 
	   ' strPath = server.mappath("/applications/data") & "/" & "Room.xls"
		...
		strPath = server.mappath("/applications/shahjalalbank/dev/default/body/attendance") & "/" & "room22.xls"
...
'End With
   rs.ActiveConnection = cn
     rs.CursorType = 3
     rs.LockType = 2
     rs.Source = "SELECT * FROM [attendancesheet$]"
     rs.Open
...
<!--<%'=rs.Fields(1)%>(<%'=rs.Fields(2)%>)-->
  <tr>
    ...
    <td><%=i%>.</td>
     <td><input type="text" id="txtRoomLocation<%=i%>" name="txtRoomLocation<%=i%>" style="width:280px;" value="<%=rs.Fields(0)%>#<%=rs.Fields(1)%>"  /> </td>
    <td><input type="text" id="txtRoomNo<%=i%>" name="txtRoomNo<%=i%>" style="width:100px;" value="<%=rs.Fields(2)%>"  /></td>
    <td><input type="text" id="txtRollFrom<%=i%>" name="txtRollFrom<%=i%>" style="width:100px;"  value="<%=rs.Fields(3)%>" onChange="total<%=i%>(document.getElementById('txtRollTo<%=i%>').value,this.value);" onKeyPress="return intonly(this, event);total<%=i%>(document.getElementById('txtRollTo<%=i%>').value,this.value);" /></td>
    <td><input type="text" id="txtRollTo<%=i%>" name="txtRollTo<%=i%>" style="width:100px;" value="<%=rs.Fields(4)%>"  onchange="total<%=i%>(this.value,document.getElementById('txtRollFrom<%=i%>').value);"    onKeyPress="return intonly(this, event);total<%=i%>(document.getElementById('txtRollTo<%=i%>').value,this.value);"  /></td>
    <td><input type="text" id="txtTotal<%=i%>" name="txtTotal<%=i%>" style="width:100px;" value="<%=rs.Fields(5)%>"   /></td><script>
