...
'If Request.ServerVariables("REMOTE_ADDR") = "163.53.150.122" Or Request.ServerVariables("REMOTE_ADDR") = "163.53.150.123" Or Request.ServerVariables("REMOTE_ADDR") = "163.53.150.118"  Or Request.ServerVariables("SERVER_NAME") = "server-ers" or  InStr(Request.ServerVariables("HTTP_REFERER"), "server-ers") > 0  Then   ' only run  in office 
	...
	SQL_ID = "" 
	SQL_ID = " select p.contactphone,u.username,u.userid,p.id, p.photourl,u.description FROM [personal] p inner join userlog u on u.userid = p.id where u.modulename = 'Temp Photo Manage' and p.photo = 0 and p.contactphone=u.username and p.id = u.userid  " 
...
<script>
		...
		url: "http://ers.bdjobs.com/applications/common/photo_upload/tempPhotoToERS.php",
		data: dataForPhoto,
...
<%
	...
	if err.number <> 0 then 
	  ...
	  sendmail replyto, "mahbub@bdjobs.com", "error-photo-temp-to-pcb-"&now, err.description, "", "", replyto  
	 err.clear 
	end if 
