...
'RESPONSE.Write(strFormDeadLine)
...
if intTrakingNo <> "" Then
	...
	if arrJob(0, 0) <> "Nothing" then
		...
		SQLDL = "select convert(varchar, appdate_deadline, 120) from jobpos where ID ="&Ref_No &" AND JOBID="& jobid
		arrSQLDL = objERS.RetrieveData(SQLDL,errMsg)
		
		str_72_after_post = arrSQLDL(0,0)
		'response.Write(str_72_after_post)			
...
If arr_Personal(0, 0) <> "Nothing" Then
	...
	DateofBirth = day(DateofBirth) &"/" & month(DateofBirth)&"/"&year(DateofBirth)
	...
	if NationalID = "" or NationalID = "0" then
		NationalID = "--"
	end if
