...
'response.Write("r: "& intAppId & "j: "& jobidF & intTrakingNo)
...
arr_Certificate = objERS.RetrieveData(SQLCertificate, errMsg)
	...
	elseif intAppId <> "" and jobidF <> "" Then	
		sql = " select jobid, id, personalid from jobpos where id = "&intAppId & " and jobid="&jobidF 
	end if
	'response.Write(sql)
	err.clear
