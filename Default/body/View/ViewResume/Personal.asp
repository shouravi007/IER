...
if strPhotoFolder = "shahjalalbank" then 
 SQLPersonal = "SELECT (FirstName + ' ' + LastName) AS Name, FatherName, MotherName, DateOfBirth, MaritalStatus, Nationality, Email, ContactPhone, CellPhone, NationalId, BloodGroup, Gender, Religion, ContactAddress, PermanentAddress, Declaration, Declaration, Reading, Writing, Speaking, Photo, Experience, ProfessionalQualification, AcademicDistinctions, ExtraCurricularActivities, ComputerSkills, OtherDisclose, HomeDistrict, photourl, jobid, FathersProfession,MothersProfession,bdjobsphoto,bdjobsid,HomeDistrict2,pass,ProfessionalCertification, [ThanaForPrmDist],[signature],[signatureurl] FROM Personal WHERE id=" & intTrackingNo
'Response.Write(SQLPersonal)
else 
 SQLPersonal = "SELECT (FirstName + ' ' + LastName) AS Name, FatherName, MotherName, DateOfBirth, MaritalStatus, Nationality, Email, ContactPhone, CellPhone, NationalId, BloodGroup, Gender, Religion, ContactAddress, PermanentAddress, Declaration, Declaration, Reading, Writing, Speaking, Photo, Experience, ProfessionalQualification, AcademicDistinctions, ExtraCurricularActivities, ComputerSkills, OtherDisclose, HomeDistrict, photourl, jobid, FathersProfession,MothersProfession,bdjobsphoto,bdjobsid,HomeDistrict2,pass,ProfessionalCertification, [ThanaForPrmDist],[signature],[signatureurl], [Training],SpouseName,SpouseProfession FROM Personal WHERE id=" & intTrackingNo
end if 
...
sql = ""
sql = "select rollno from roll where personalid ="&intTrackingNo
ValuePrint sql, "sql"
...
SQL_ID = "" 
SQL_ID = "select id from jobpos where  personalid = " & intTrackingNo 
...
'	End If
	...
	strSpouseName = arr_Personal(41, 0)
	strSpouseProfession = arr_Personal(42, 0)
...
End If
	...
	strSignatureSrc = arr_Personal(39, 0) '&"?"&time&rnd 
      ...
      %>
          ...
          -->
          <!--include virtual = '/applications/common/photo_upload_ers/photo_view.asp' -->  
