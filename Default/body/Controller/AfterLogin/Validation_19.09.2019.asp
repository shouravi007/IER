...
If IsEdit = True Then 
	...
	If TrackNoNew <> "" and strPassword <> "" Then
		...
		If system = "ksb" Then					
			sql = "select personalid, id, jobid, DATEDIFF(HOUR,appdate_deadline, GETDATE()) from jobpos where [personalid] = "&TrackNoNew			
		Else
		...
		if arrID(0, 0) <> "Nothing" Then
			...
			If system = "ksb" Then	
				...
				intAppId = arrID(0, 0)
				'RESPONSE.Write(intAppId)
				
				strAppdateDeadline = arrID(3, 0)
				'response.Write(strAppdateDeadline)
				
				If strAppdateDeadline > 3 then ' 3 days/72 hours
					...
					ErrorMessage "66", errMsg, 0
				
					if arr_sqlCheckPaymentTable(0,0) = "Nothing" Then
%>

					<script type="text/javascript">					
                        alert("Your Application is locked because you didn't pay your application fee within 72 hours.\nIf you want to unlock your resume, please click on apply new button for re-apply and pay your application fee within 72 hrs.");
                    </script>
...
<%                    
			...
			SQLPersonal = "SELECT id, (FirstName+' '+LastName) as Name, Email,ContactPhone, CellPhone, Gender, PostedOn, UpdatedOn, Pass, Photo, photoURL,jobid,bdjobsphoto,bdjobsid, DateOfBirth,signatureurl, SUBSTRING(LEFT(replace(photourl,'http://my.bdjobs.com/photos/',''), LEN(replace(photourl,'http://my.bdjobs.com/photos/',''))-4), CHARINDEX('/', LEFT(replace(photourl,'http://my.bdjobs.com/photos/',''), LEN(replace(photourl,'http://my.bdjobs.com/photos/',''))-4))+1, LEN(LEFT(replace(photourl,'http://my.bdjobs.com/photos/',''), LEN(replace(photourl,'http://my.bdjobs.com/photos/',''))-4))) FROM Personal WHERE id=" & TrackNo & "  and postedon >= '2019-09-01'  " 
