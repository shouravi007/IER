...
'	end if
	...
	If system = "ksb" Then	
		...
		if arrJobPOS(0, 0) <> "Nothing" Then 
			...
			totalAppliedJob = ubound(arrJobPOS,2)
			
			for jj = 0 to totalAppliedJob
								 
				're apply parameter					
				
				if totalAppliedJob = 0 then
				
					intAppId = arrJobPOS(0,0)
					intJobIdA = arrJobPOS(1,0)
					strAppdateDeadline = arrJobPOS(4,0)
				...
				'response.Write("strAppdateDeadline: :"&strAppdateDeadline)				
				...
				If intAppId <> "" and strAppdateDeadline >= 0 then ' 3 days/72 hours strAppdateDeadline > "3" 								
					...
					if arr_sqlCheckPaymentTable(0,0) = "Nothing" Then
						...
						if intJobIdA = 1 then
							...
							strMsgApply2 = "Assistant Officer (General) "
							reAppJobId = 1	
						elseif intJobIdA = 2 then
							jobPosName = "Assistant Officer (Cash)"
							strMsgApply2 = "Assistant Officer (Cash) "
							reAppJobId = 2
						end if
						
						strBlockMessage = "Your Application is locked because you didn't pay your application fee within 72 hours. If you want to unlock your resume, please click on Re-Apply button for new apply and pay your application fee within 72 hours.<br>"
