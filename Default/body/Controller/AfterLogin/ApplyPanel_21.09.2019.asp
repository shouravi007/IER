...
'apply parameter 
	...
	If system = "ksb" Then	
		...
		sqlJobPosCheck = "select jp.id, jp.JOBID, jp.AppDate, jp.appdate_deadline,  DATEDIFF(HOUR,appdate_deadline, GETDATE()) from jobpos jp inner join personal p on jp.personalid = p.id  where jp.personalid ="&TrackNo
		...
		if arrJobPOS(0, 0) <> "Nothing" Then 
			...
			for jj = 0 to ubound(arrJobPOS,2)
				...
				If intAppId <> "" and strAppdateDeadline > 3 then ' 3 days/72 hours strAppdateDeadline > "3" 
