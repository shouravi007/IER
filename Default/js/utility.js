 ...
 function EmailTypeInputValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName)
 {
	...
	if(Required == "True")
       {  
			  ...
			  if (!filter.test(document.getElementById(strVariable).value.trim())) 
			 {
				   ShowValidateMsg( "Invalid " + strMsgVariableName + " address .", strVariable);
				return false;
			 }else{
				 //strVariable.style.backgroundColor = fgcolor;
				 document.getElementById(strVariable).style.borderColor = "Orange";
				 return true;
	  ...
	  {
		  if(document.getElementById(strVariable).value != "")
		  {
				if (filter.test(document.getElementById(strVariable).value.trim()))
				{			
					document.getElementById(strVariable).style.backgroundColor = fgcolor;
					return true;
				}
				else 
				{
...
function  PasswordTypeInputValidation(Required,strVariable,FormInputType,DbType,DbFieldSize,strMsgVariableName,clientTop)
{
	if(Required == "True")
	{
	var strPassword = document.getElementById(strVariable);
	var strConfirmpassword = document.getElementById(DbType);
	  ...
	  var DbFieldSize = parseInt(DbFieldSize);
	    ...
	    if(strPassword.value.trim() == strConfirmpassword.value.trim())
		  {
					 ShowValidateMsg( strMsgVariableName+"  are not same ! ",strVariable,clientTop );
					 return false;
		  }
...
function PhotoTypeInputValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName,clientTop)
	...
	if ( strVariable == "" || strVariable.length == 0 )
	  { 
	     // ShowValidateMsg(" Please upload a " + clientTop, document.getElementById('File1'),clientTop);
	      ...
	      ShowValidateMsg(" Please upload a " + clientTop, 'File1',clientTop);
 ...
 function CertificateTypeInputValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName,clientTop)
  {
 if ( Required == "True" ) 
   {
	if ( strVariable == "" || strVariable.length == 0 )
	  { 
	     // ShowValidateMsg2(" Please upload a "+clientTop, document.getElementById('File_Certificate'),clientTop);
		  
	      ShowValidateMsg(" Please upload a "+clientTop,'File_Certificate',clientTop);
...
function SelectTypeInputValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName,clientTop)
 {
   if(Required == "True")
       {  
/*	   alert(strVariable.value);
*/       if(document.getElementById(strVariable).value == DbType || document.getElementById(strVariable).value == "Bangladesh" )
	     { 
...
function CheckValidation(Required,strVariable,FormInputType,DbType,DbFieldSize,strMsgVariableName,clientTop)
{
	 ...
	 try 
	 {
		 switch(FormInputType)
		 {
			  ...
			  case "Certificate":
			     txt = CertificateTypeInputValidation(Required,strVariable,FormInputType,DbType,DbFieldSize,strMsgVariableName,clientTop);
			   break;
		 ...
		 {
		 correctVariable = checkCopyPestFromWord(document.getElementById(strVariable));
	//	 return correctVariable;
      	 document.getElementById(strVariable).style.borderColor = txtcolor;
		 }
		 }
	 }
	 catch(ex)
...
function TextTypeInputValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName,clientTop)
{
	 ...
	 var obj = document.getElementById(strVariable).value.trim();
...
function  RequiredTextTypeDataValidation(Required,strVariable,DbType,DbFieldSize,strMsgVariableName,clientTop)
 {
      var strDataType = DbType.toString(); 
			   switch(strDataType)
			     { 
				 case "Text":
					  var strLength =  document.getElementById(strVariable).value.trim().length;
                     // alert(strVariable);
					  var DbFieldSize = parseInt(DbFieldSize);
				  ...
				  case  "Number":
				     if(document.getElementById(strVariable).value !="")
					  {
                        if (isNumber(document.getElementById(strVariable).value.trim()) == false)
						  {							   
...
function CGPATypeInputValidation(Required,strCgpaOrMarks,cgPaScale,strMsgVariableName,obj,row)	
  {
...
*/						
		...
		else
		{
			...
			{
				 ShowValidateMsg("CGPA/Marks can't be less than " +resultRangeSSCHSC+ " in row " + row+ ".",obj,row);
				 return false;
			}
		}
     }
...
//    popup(strMsg,clientTop);
   // alert(strMsg);
        ...
        $('#' + obj + '_message').text(strMsg);
        if ( $('#' + obj + '_message').is(':hidden') ) {
          $('#' + obj + '_message').show('medium');
        }	
